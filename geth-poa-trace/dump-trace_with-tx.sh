...
LAST_TRACE=$(ls -v "$TRACE_FOLDER" | grep "trace_with_tx_" | tail -n 1 | awk -F '_' '{print $4}' | awk -F '.' '{print $1}')
...
TRACE_FILE="$TRACE_FOLDER/trace_with_tx_$NEXT_TRACE.out"

# Create the trace file and execute the commands
curl 172.13.0.100:8080/start-trace
cast send 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266 --value 0.00001ether --private-key $PRIVATE_KEY
